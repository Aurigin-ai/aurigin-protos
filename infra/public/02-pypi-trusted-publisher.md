...
Wire `pypi.org` to accept uploads from the `publish-pypi.yml`
...
- The `publish-pypi.yml` workflow filename agreed (this runbook
  assumes `publish-pypi.yml`; change consistently below if you pick
  a different name).
- A `public-release` GitHub Environment exists on the repo. It does
  NOT need required reviewers — we use it only so the OIDC token has
  an `environment` claim that this binding can match against.
...
`publish-pypi.yml` runs in the `public-release` environment on
...
workflow once. Until step 03 + step 04 + `publish-pypi.yml` are in
...
3. After `publish-pypi.yml` exists and runs successfully once,
...
## What `publish-pypi.yml` needs on the workflow side
...
- **Workflow filename mismatch.** PyPI matches on the exact filename,
  not the workflow `name:` field. If the file is `publish-pypi.yml`,
  the config must say `publish-pypi.yml`. Rename one without the
  other and uploads fail with "OIDC token claims do not match".
- **Environment claim missing.** If `publish-pypi.yml` doesn't set
...
- **Pending publisher consumed on the wrong project.** If
  `publish-pypi.yml` (for whatever reason) uploads a *different*
